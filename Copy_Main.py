...
def encrypt_image_BGR(image_path: str):
    image = cv2.imread(image_path, cv2.IMREAD_ANYCOLOR)
...
def main():
    ...
    image_path = "/Users/nguyenminh/Documents/new_PLCM/1045-2.jpg"
 ...
 #   cv2.imwrite("encrypted.png", encrypted)
    ...
    B, G, R = cv2.split(img)
    encrytped_image_color = encrypt_image_BGR(image_path)
    end_time =time.time()
    cv2.imwrite('encrytped_image_BGR.png',encrytped_image_color)
    cv2.imshow('img',img)
    cv2.imshow('encrytped_color',encrytped_image_color)
  #  cv2.imshow('Blue',B)
   # cv2.imshow('Green', G)
   # cv2.imshow('Red',R)
   # cv2.imshow('Encrypted_B',encrytpedBGR[0])
   # cv2.imshow("Encrypted_G",encrytpedBGR[1])
   # cv2.imshow("Encrypted_R", encrytpedBGR[2])
